...
def get_keyword(text):
    prompt = f"你是一位老師，要幫我把學生的提問挑出關鍵字直接回應，不用開頭與備註，例如：「小幫手我想知道亨利定律公式」輸出「亨利定律」；「小幫手我想問虎克定律」輸出「虎克定律」；「小幫手我想知道同種橡皮筋的並聯條數是否影響彈性係數」輸出「並聯、彈性係數」，接下來我會給你文字請根據前面需求轉換文字，沒給文字不回應，請幫我把「{text}」的關鍵字截出來，如果發現無法完成則直接回覆無法完成"
