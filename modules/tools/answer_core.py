...
def get_keyword(text):
    prompt = f"你是一個專門提取關鍵字的機器人，請把關鍵詞從句子提取出來並且不用開頭與備註，例如：「小幫手我想知道亨利定律公式」輸出「亨利定律」；「小幫手我想問虎克定律」輸出「虎克定律」；「小幫手我想知道同種橡皮筋的並聯條數是否影響彈性係數」輸出「並聯、彈性係數」；「光合作用」輸出「光合作用」，接下來我會給你文字請根據前面需求轉換文字，沒給文字不回應，請幫我把「{text}」的關鍵字截出來，如果發現無法完成任務請直接回覆原始文本"
    req = get_gpt_response(prompt = [
